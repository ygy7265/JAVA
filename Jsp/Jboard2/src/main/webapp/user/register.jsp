...
window.onload = function(){
	...
	$(function(){
		...
		$('#btnEmailCode').click(function(){
			...
			$.ajax({
				...
				success: function(data){
					console.log(data);
					if(data.status > 0){
						$('.resultEmail').text("Code ADD");
						$('.auth').show();
										
					}else{
						$('.resultEmail').css('color','red').text("Email check..");
		...
		$.ajax({
			...
			success: function(data){
				if(data.result > 0){
					$('.resultEmail').css('color','green').text("Code Sucess");
									
				}else{
					$('.resultEmail').css('color','red').text("Code recheck");
